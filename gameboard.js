...
sock.on("player-info", (package) => {
    ...
    if (package.partyrole == "Liberaal"){
        ...
        document.getElementById("Fascisten_Box").innerHTML = package.fascists;
...
function ChoosePresidentPolicy(id){
    ...
    element.classList.add("verdhttp://localhost:8080/index.htmlwijnen");
...
//functie waarmee een speler kan worden neergeschoten
document.getElementById("Shoot-form").addEventListener("submit", (e) => {
    //'preventDefault' => zorgt ervoor dat de pagina niet wordt herladen.
    e.preventDefault()
    var choice = document.getElementById("Shoot-vote").value;
    console.log(`Je hebt ${choice} gekozen`);
    sock.emit("game-shoot-choice", choice);
});

function Shoot_function(clicked_id){
    var element = document.getElementById(clicked_id);
    element.classList.remove("verschijnen");
    element.classList.add("verdwijnen");
    document.getElementById("player0-container").removeAttribute("onclick", "Shoot_function(this.id)");
    document.getElementById("player1-container").removeAttribute("onclick", "Shoot_function(this.id)");
    document.getElementById("player2-container").removeAttribute("onclick", "Shoot_function(this.id)");
    document.getElementById("player3-container").removeAttribute("onclick", "Shoot_function(this.id)");
    document.getElementById("player4-container").removeAttribute("onclick", "Shoot_function(this.id)");

}

function Add_Function(){
document.getElementById("player0-container").setAttribute("onclick", "Shoot_function(this.id)");
document.getElementById("player1-container").setAttribute("onclick", "Shoot_function(this.id)");
document.getElementById("player2-container").setAttribute("onclick", "Shoot_function(this.id)");
